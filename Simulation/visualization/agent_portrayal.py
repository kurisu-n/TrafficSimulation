...
def agent_portrayal(agent):
    ...
    """
    # Case 1: Real cell – city layer
    if isinstance(agent, CellAgent):
        por = agent.get_portrayal()
        por["LayerName"] = "City Layer"
        return por

    # Case 2: Real vehicle – vehicle layer
    if isinstance(agent, VehicleAgent):
        por = agent.get_portrayal()
        por["LayerName"] = "Vehicle Layer"
        return por

    # Case 3: Dummy – show underlying cell info on vehicle layer
    if isinstance(agent, DummyAgent):
        ...
        filtered["Layer"] = 1
        filtered["Type"] = "Vehicle Layer"
        filtered["LayerName"] = "Vehicle Layer"
        return filtered
